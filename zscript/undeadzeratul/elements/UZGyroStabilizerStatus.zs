class UZGyroStabilizer : HUDElement {
    ...
    override void DrawHUDStuff(HCStatusbar sb, int state, double ticFrac) {
        ...
        if (CheckCommonStuff(sb, state, ticFrac)) {
            ...
            string s = GetStabilizerStatusText(sb.hpl);
            int t = GetStabilizerStatusTimeout(sb.hpl);
            if (s != "" && t > 0) {
                sb.drawstring(
                    _hudFont,
                    s,
    ...
    private string GetStabilizerStatusText(PlayerPawn p) {
        service GyroStatus = ServiceIterator.Find("UaS_GyroStabilizerStatus").next();
        return GyroStatus ? GyroStatus.GetStringUI("StatusText", objectArg:p) : "";
    }
    
    private int GetStabilizerStatusTimeout(PlayerPawn p) {
        service GyroStatus = ServiceIterator.Find("UaS_GyroStabilizerStatus").next();
        return GyroStatus ? GyroStatus.GetIntUI("StatusTimeout", objectArg:p) : -1;
    }
}
